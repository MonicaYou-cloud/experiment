...
if st.session_state.page == 0:
    st.header("歡迎參加本測驗")
    st.markdown("---")
    st.write("您好！首先非常感謝您願意參與本測驗，本測驗為政治大學心理學研究所進行的碩士學位研究。研究目的是想要了解受試者在做智力測驗前的練習與正式測驗分數有何關聯。
    本測驗採用現有且已被證實有效的智力測驗作為測驗材料，共分為兩階段：練習階段與正式測驗階段。當您開始進行本測驗，需要輸入受試者編號以便與實驗主機連線。
    此外也將會需要填寫簡易的基本資料表（含性別、年齡），以及關於您的一些價值觀和想法，填寫完畢後將進入測驗的兩階段。
    您有完全自主性，可以自由決定是否參與以及繼續本測驗，若您感到不適，可以隨時中止測驗。
    本研究不會對您造成任何風險，您所填寫的資料將完全保密。資料收回後將由研究人員電子歸檔與保存並進行統計數據分析，預計保留5年後全數刪除，請您放心。
    填答完畢者可參加抽獎，獎項為XXX，研究者將會從後台對照受試者編號並將獎品寄到填寫表單之電子郵件。
    若您是需要換取課堂學分則需要參與現場實驗，且無法參加抽獎活動，謝謝！
    最後，如果您有任何疑問，可以直接聯繫研究人員（112752003@g.nccu.edu.tw）
    國立政治大學 心理學系
    碩士學生 游主雲
    指導老師 孫蒨如 教授
    敬上
