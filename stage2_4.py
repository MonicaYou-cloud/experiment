...
@st.cache_resource
def get_sheet():
    return client.open("experiment_data").worksheet("stage2-4")
...
participants = {
    ...
    "EYGT24": [datetime(2025, 9, 13, 20, 30, 0, tzinfo=tz), datetime(2026, 9, 15, 21, 30, 0, tzinfo=tz)],
    "JurM24": [datetime(2025, 9, 16, 10, 0, 0, tzinfo=tz), datetime(2026, 9, 16, 11, 0, 0, tzinfo=tz)],
    "OetM24": [datetime(2025, 9, 16, 13, 30, 0, tzinfo=tz), datetime(2026, 9, 16, 14, 30, 0, tzinfo=tz)],
    "RaYx24": [datetime(2025, 9, 15, 14, 0, 0, tzinfo=tz), datetime(2026, 9, 15, 15, 0, 0, tzinfo=tz)],
    "ZYrQ24": [datetime(2025, 9, 16, 10, 0, 0, tzinfo=tz), datetime(2026, 9, 16, 11, 0, 0, tzinfo=tz)],
    "dFsQ24": [datetime(2025, 9, 16, 13, 30, 0, tzinfo=tz), datetime(2026, 9, 16, 14, 30, 0, tzinfo=tz)],
    "gdFs24": [datetime(2025, 9, 15, 14, 0, 0, tzinfo=tz), datetime(2026, 9, 15, 15, 0, 0, tzinfo=tz)],
    "pXby24": [datetime(2025, 9, 16, 10, 0, 0, tzinfo=tz), datetime(2026, 9, 16, 11, 0, 0, tzinfo=tz)],
    "qfVw24": [datetime(2025, 9, 16, 13, 30, 0, tzinfo=tz), datetime(2026, 9, 16, 14, 30, 0, tzinfo=tz)],
    "wqyA24": [datetime(2025, 9, 16, 13, 30, 0, tzinfo=tz), datetime(2026, 9, 16, 14, 30, 0, tzinfo=tz)],
}
...
# 練習說明
    ...
    st.write("""在本階段的努力程度指的是練習時間長度。""")
    st.write("""建議您練習【11~15題】後，就可以點選〔直接進入正式測驗〕。""")
...
if st.session_state.page == 123:
    ...
    new_seconds = int(total_seconds * 2 - 25)
...
if st.session_state.page == 124:
    ...
    st.write("１. 與同齡人平均的練習時間相比，您的練習時間更多還是更少？")
    prac1 = st.radio(
